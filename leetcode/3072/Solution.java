...
class Solution {
    public int[] resultArray(int[] nums) {
        ...
        Arrays.sort(sorted);
        SegmentTreeLazyMark segment1 = new SegmentTreeLazyMark(new int[n]);
        SegmentTreeLazyMark segment2 = new SegmentTreeLazyMark(new int[n]);
        List<Integer> arr1 = new ArrayList<>();
        List<Integer> arr2 = new ArrayList<>();
        arr1.add(nums[0]);
        int index1 = Arrays.binarySearch(sorted, nums[0]);
        segment1.update(index1, index1, 1, 1);
        arr2.add(nums[1]);
        int index2 = Arrays.binarySearch(sorted, nums[1]);
        segment2.update(index2, index2, 1, 1);
        for (int i = 2; i < n; i++) {
            int index = Arrays.binarySearch(sorted, nums[i]);
            int greaterCount1 = 0;
            int greaterCount2 = 0;
            if (index < n - 1) {
                greaterCount1 = segment1.getSum(index + 1, n - 1, 1);
                greaterCount2 = segment2.getSum(index + 1, n - 1, 1);
            }
            if (greaterCount1 > greaterCount2 || (greaterCount1 == greaterCount2 && arr1.size() <= arr2.size())) {
                // arr1
                arr1.add(nums[i]);
                segment1.update(index, index, 1, 1);
            } else {
                // arr2
                arr2.add(nums[i]);
                segment2.update(index, index, 1, 1);
            }
        }
        arr1.addAll(arr2);
        return arr1.stream().mapToInt(o -> o).toArray();
    }
    ...
    public static void main(String[] args) {
        ...
        int[] nums = {5,14,3,1,2};
        System.out.println("test: " + Arrays.toString(sol.resultArray(nums)));
    }
}