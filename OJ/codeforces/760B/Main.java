...
public class Main {
    ...
    private static boolean check(int remain, int n, int k, int target) {
        if (target == 0) return true;
        // index from 0
        k = k - 1;
        int leftLen = Math.min(k + 1, target);
        long leftSum = ((long)leftLen * (target - leftLen + 1 + target)) / 2;
        int rightLen = Math.min(n - k, target);
        long rightSum = ((long)rightLen * (target - rightLen + 1 + target)) / 2;
        long totalSum = leftSum + rightSum - target;
        return totalSum <= remain;
    }
